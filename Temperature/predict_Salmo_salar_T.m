...
%    % tLps bis no bertalanffy
   ...
   E0 = L0^3 * p_Am/ v; H0 = E_H;
%    f1 = f_ShelClar1995; f0 = 0.6 * f_ShelClar1995; f2 = 0.8 * f_ShelClar1995;  % -, set food levels
   ELH_0 = [f_tl * E0; L0; H0];% ELH_1 = [f1 * E0; L0; H0]; ELH_2 = [f2 * E0; L0; H0]; % state vector at start

   [a ELH] = ode45(@dget_ELH_pj, [0; tLps(:,1)], ELH_1, [], L_b, L_j, L_m, p_Am, v, g, k_J, kap, f_tl, E_Hb, E_Hj, TC_tLps, T_A, T_ref); 
   ELH(1,:) = []; E = ELH(:,1); L = ELH(:,2); ELps = L/ del_M;  EWw_ShelClar1995_1 = L.^3 + E * w_E/ mu_E/ d_E;  % g, wet weight
...
function dELH = dget_ELH_pj(t, ELH, Lb, Lj, Lm, p_Am, v, g, kJ, kap, f, Hb, Hj, tT, T_A, T_ref)
  ...
  dL = rT * L/3;
  dH_j = (1 - kap) * pT_C - kT_J * H;
  dH_r = (1 - kap) * pT_C - kT_J * H;
